...
def test_mysql_missing_binary_raises():
    ...
    with patch("shutil.which", return_value=None), pytest.raises(Exception, match="mysqldump"):
...
def test_mysql_backup_missing_binary_raises():
    ...
    with patch("shutil.which", return_value=None), pytest.raises(Exception, match="mysqldump"):
        MySQLAdapter().backup(MagicMock(host="h", user="u", password="p", database="db", port=3306))
