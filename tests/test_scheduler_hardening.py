...
def test_scheduler_active_job_detection():
    from dbbackup.core.scheduler import SchedulerDaemon
    from apscheduler.schedulers.background import BackgroundScheduler
    from apscheduler.jobstores.memory import MemoryJobStore
    from apscheduler.executors.pool import ThreadPoolExecutor
    from dbbackup.core.scheduler import _make_job_runner
    sched = BackgroundScheduler(jobstores={"default": MemoryJobStore()}, executors={"default": ThreadPoolExecutor(max_workers=2)}, job_defaults={"coalesce": True, "max_instances": 1, "misfire_grace_time": 300})
    daemon = SchedulerDaemon(sched, shutdown_grace_seconds=4)
    daemon.scheduler.start()
    try:
        runner = _make_job_runner("j1", lambda: MagicMock(connection=MagicMock(db_type="sqlite", database="d")), daemon_ref=daemon._active_jobs)
        # inject into scheduler
        from datetime import datetime, timedelta
        from apscheduler.triggers.date import DateTrigger
        sched.add_job(runner, trigger=DateTrigger(run_date=datetime.now() + timedelta(milliseconds=50)), id="j1", replace_existing=True)
        # start job quickly
        time.sleep(0.2)
        ...
        with patch("dbbackup.core.scheduler.run_backup", side_effect=_slow_result(0.8)):
            runner2 = _make_job_runner("j1b", lambda: MagicMock(connection=MagicMock(db_type="sqlite", database="d")), daemon_ref=daemon._active_jobs)
...
def test_scheduler_grace_waits_within_period():
    ...
    with patch("dbbackup.core.scheduler.run_backup", side_effect=slow):
        config = {"schedule": {"jobs": [{"id": "a", "interval_seconds": 9999, "db_type": "sqlite", "database": "d", "s3_bucket": "b"}], "shutdown_grace_seconds": 3}}
...
def test_scheduler_no_active_jobs_immediate():
    ...
    with patch("dbbackup.core.scheduler.run_backup"):
        config = {"schedule": {"jobs": [{"id": "a", "interval_seconds": 9999, "db_type": "sqlite", "database": "d", "s3_bucket": "b"}], "shutdown_grace_seconds": 2}}
        daemon = start_scheduler(config)
        try:
            daemon.request_shutdown()
            t0 = time.monotonic()
...
def test_scheduler_mixed_local_s3_jobs():
    from dbbackup.core.scheduler import start_scheduler
    with patch("dbbackup.core.scheduler.run_backup", return_value=MagicMock(status="success")) as mock_run:
        config = {"schedule": {"jobs": [
            {"id": "local_job", "interval_seconds": 9999, "db_type": "sqlite", "database": "d", "storage": "local", "local_path": "/tmp"},
            {"id": "s3_job", "interval_seconds": 9999, "db_type": "sqlite", "database": "d", "s3_bucket": "b"},
